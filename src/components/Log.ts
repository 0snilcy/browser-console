...
export default class Log {
  ...
  private getOriginalPosition(lineNumber: number, columnNumber: number) {
    const { line, column, source } = this.sourceMaps.getOriginalPosition(
      this.url,
      {
        line: lineNumber + 1,
      ...
      .map((propertyDescriptor) => {
        ...
        return {
          ...
          descriptors: descriptors
            .filter((key) =>
              Object.hasOwnProperty.call(propertyDescriptor, key)
            )
            .reduce(
              (obj: IDescriptor, key) => (
                (obj[key] = propertyDescriptor[key]), obj
              ),
              {}
            ),
        };
  ...
  private foramtInternalProps(
    props: Protocol.Runtime.InternalPropertyDescriptor[]
  ) {
    return props
      .filter(({ value }) => value)
      .map((propertyDescriptor) => {
  ...
  async getProps(object: IPreview): Promise<IGetPropsResponse[] | undefined> {
    if (object.objectId) {
      const ownPropsResponse =
        (await this.getPropsByObjectId(object.objectId)) || {};
      const propsResponse =
        (await this.getPropsByObjectId(object.objectId, false)) || {};
  ...
  private entryToString(entry: Protocol.Runtime.EntryPreview | undefined) {
    if (entry?.value) {
      if (entry.key) {
        return `${this.objectPreviewToString(
          entry.key
        )} => ${this.objectPreviewToString(entry.value)}`;
      }
